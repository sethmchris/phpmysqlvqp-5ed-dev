...
<body>
<?php # Script 13.3 - Modified version of upload_rtf.php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_FILES['upload']) && file_exists($_FILES['upload']['tmp_name'])) {
		...
		if (finfo_file($fileinfo, $_FILES['upload']['tmp_name']) == 'text/rtf') {
			...
			# 4. Modify upload_rtf.php so that it reports the actual MIME type for the uploaded file, should it not be text/rtf.
		} else { // Invalid type.
			echo '<p style="font-weight: bold; color: #C00">You uploaded the following filetype: ' . finfo_file($fileinfo, $_FILES['upload']['tmp_name']) . '<br />Please upload an RTF document.</p> ';
		}
